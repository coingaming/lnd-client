...
lnd --debuglevel="RPCS=trace" --lnddir=$LND_MERCHANT_DIR > $THIS_DIR/.lnd-merchant/stdout.log & echo $! > "$LND_MERCHANT_DIR/lnd.pid"
...
lnd --debuglevel="RPCS=trace" --lnddir=$LND_CUSTOMER_DIR > $THIS_DIR/.lnd-customer/stdout.log & echo $! > "$LND_CUSTOMER_DIR/lnd.pid"
